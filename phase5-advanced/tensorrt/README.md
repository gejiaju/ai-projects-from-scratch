...
> 面试可讲：我做部署对比时发现 ONNX 反而慢，排查后定位到是装了 CPU 版 onnxruntime、
> 没启用 GPU provider。这让我理解了"推理引擎要选对后端"这个部署的关键点。

## 面试自述要点

"我做了一个模型部署的对比：把训练好的 YOLOv8 导出成 ONNX，对比 PyTorch 和 ONNX 的
推理速度，画出精度-速度的权衡曲线。核心思路是——训练追求精度，部署追求速度，中间
有个权衡。TensorRT 更进一步，它针对特定 GPU 做编译期优化，把算子融合、精度降为 FP16/INT8，
速度最快但需要重新编译。这和我本科学的功耗-性能权衡是一个道理。"
